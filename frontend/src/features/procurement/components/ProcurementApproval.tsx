...
import { 
  ...
  ArrowLeft, Route as RouteIcon, CheckCircle, XCircle, X, Pen
} from 'lucide-react';
...
const ProcurementApproval: React.FC = () => {
  ...
  const handleApprove = async () => {
    if (!id) return;
    if (window.confirm('确认通过该采购申请吗？')) {
      try {
        await approveProcurement(parseInt(id), comment);
        // Trigger menu update
  ...
  const handleReject = async () => {
    ...
    if (window.confirm('确认拒绝该采购申请吗？')) {
      try {
        await rejectProcurement(parseInt(id), comment);
        // Trigger menu update
        window.dispatchEvent(new Event('menu-update'));
        alert('已拒绝该申请');
        navigate('/');
    ...
    <div className="p-8">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <div>
            <h1 className="text-2xl font-semibold text-gray-800 mb-1">采购申请审批</h1>
            <p className="text-sm text-gray-500">审批采购申请，确认信息无误后点击通过或拒绝</p>
          </div>
        </div>
      </div>
      ...
      <div id="form-section" className="space-y-6">
        ...
        {/* Comment Section */}
        {procurementRequest?.status === 'APPROVING' && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-center mb-4">
                <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center mr-3">
                    <Pen className="text-blue-600 w-5 h-5" />
                </div>
                <h2 className="text-lg font-semibold text-gray-800">审批意见</h2>
            </div>
            <textarea
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                className="w-full h-32 p-4 border border-gray-300 rounded-md bg-white text-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                placeholder="请输入审批意见（通过时选填，拒绝时必填）..."
            ></textarea>
            </div>
        )}
            ...
            {procurementRequest?.status === 'APPROVING' && (
                <>
                    <button 
                        ...
                        className="px-6 py-2.5 text-sm text-white bg-red-600 rounded-md hover:bg-red-700 font-medium flex items-center shadow-sm"
                    >
                        <XCircle className="mr-2 w-4 h-4" /> 拒绝
                    </button>
                    <button 
                        onClick={handleApprove}
                        className="px-6 py-2.5 text-sm text-white bg-green-600 rounded-md hover:bg-green-700 font-medium flex items-center shadow-sm"
                    >
                        <CheckCircle className="mr-2 w-4 h-4" /> 审批通过
                    </button>
                </>
            )}
            {procurementRequest?.status === 'APPROVED' && (
                <span className="px-6 py-2.5 text-sm text-green-700 bg-green-50 rounded-md font-medium flex items-center border border-green-200">
                    <CheckCircle className="mr-2 w-4 h-4" /> 已通过
                </span>
            )}
            {procurementRequest?.status === 'REJECTED' && (
                <span className="px-6 py-2.5 text-sm text-red-700 bg-red-50 rounded-md font-medium flex items-center border border-red-200">
                    <XCircle className="mr-2 w-4 h-4" /> 已拒绝
                </span>
            )}
          </div>
        </div>
